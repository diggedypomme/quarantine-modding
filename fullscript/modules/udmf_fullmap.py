...
def make_udmf(quarantine_folder,output_folder,city):
    ...
    make_multiple(map_definitions, udmf_map,  object_pre_num , wall_pre_num,city,tracking_dict  )
...
def open_map(mapname,original_gamedata ):
    print(mapname)
    print(original_gamedata)
    map_file="{}/{}.MAP".format(original_gamedata,mapname.upper())
    ...
    # Load the JSON data into a Python dictionary
        ...
        width = struct.unpack('<H', map_file.read(2))[0]  # UINT16LE for width
        height = struct.unpack('<H', map_file.read(2))[0]  # UINT16LE for height
        print(f"Map Width: {width}, Map Height: {height}")
        
        # Read the tiles
        tile_data = []
        for _ in range(width * height):
            tile = struct.unpack('<H', map_file.read(2))[0]  # UINT16LE for each tile
...
if __name__ == "__main__":
    ...
    #mapdata=open_map("jcity",original_gamedata)
    #print(mapdata)
    
    
    file_path="C:/2025_projects/Quarantine_fullset/Original_Game/JCITY.MAP"
    #make_udmf(output_folder,"jcity",original_gamedata)
    width, height, tiles_2d = read_map_file(file_path)
    ...
    # Write UDMF content to a text file
    #with open("udmf_data_.txt", "w") as file:
    #    file.write(udmf_content)
